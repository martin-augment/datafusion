...
impl ApproxTopKAccumulator {
    fn new(k: usize) -> Self {
        Self::new_with_data_type(k, None)
    }
...
impl Accumulator for ApproxTopKAccumulator {
    ...
    fn state(&mut self) -> Result<Vec<ScalarValue>> {
        let summary_bytes = self.summary.to_bytes();
        let data_type_str = Self::data_type_to_string(
            self.input_data_type.as_ref().unwrap_or(&DataType::Utf8),
        );

        Ok(vec![
            ScalarValue::Binary(Some(summary_bytes)),
            ScalarValue::UInt64(Some(self.k as u64)),
            ScalarValue::Utf8(Some(data_type_str)),
        ])
    }
...
mod tests {
    ...
    fn test_accumulator_merge_batch() {
        let mut acc1 = ApproxTopKAccumulator::new(3);
        let mut acc2 = ApproxTopKAccumulator::new(3);
    ...
    fn test_distributed_merge_simulation() {
        let mut worker1_acc = ApproxTopKAccumulator::new(3);
        let mut worker2_acc = ApproxTopKAccumulator::new(3);
        let mut worker3_acc = ApproxTopKAccumulator::new(3);
        ...
        let mut coord_acc = ApproxTopKAccumulator::new(3);
